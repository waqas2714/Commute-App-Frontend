...
const ReviewUser = ({ image, driverName, from, to, reviewId, setReviews }) => {
  ...
  const addReview = async (e) => {
    ...
    if (comment === "") {
      return toast.error(
        "Please write something to submit the review.",
        toastOptions
      );
    }
    try {
      ...
      const { data } = await axios.post(
        `${backendUrl}/api/reviews/addReview/${reviewId}`,
        { comment }
      );
  ...
  return (
    <>
      ...
      {isReviewModalOpen && (
        <div
          className="fixed top-[10vh] left-0 flex justify-center items-center w-screen h-[90vh] bg-[rgba(0,0,0,0.7)] z-10"
          onClick={() => setIsReviewModalOpen(false)}
        >
          <form
            onSubmit={addReview}
            className="w-[90vw] max-w-[610px] h-[35vh] p-4 bg-[#4CE5B1] text-black flex flex-col justify-around items-center"
          ...
          >
            ...
            <button
              type="submit"
              className="bg-black text-white rounded-full p-2 mt-1"
            >
              Submit!
            </button>
          </form>
      ...
      <div
        className="flex bg-[#161616] w-full sm:w-[350px] text-white p-2 mt-2 hover:bg-[#4CE5B1] group transition-all ease-in-out duration-150 cursor-pointer"
        onClick={() => {
          navigator.onLine
            ? setIsReviewModalOpen(true)
            : toast.error(
                "Please add your review when you are online.",
                toastOptions
              );
        }}
      >
